...
const URL_TVShows: string =
  'https://api.themoviedb.org/3/tv/popular?api_key=' +
  API_KEY +
  '&language=en-US&page=1';

const URL_TVShow_PROFILE = (id: number) =>
...
export async function GetListTVShows() {
  const response = await axios.get(URL_TVShows).catch(function (error) {
    // handle error
    console.log(error);
  });
  const results = [];
  ...
  for (const item in response.data.results) {
    ...
    results.push(tvshowsObject);
  }

  return results;
}

export async function GetTVShowProfile(id: number) {
  const url = URL_TVShow_PROFILE(id);
